...
(defn app []
  (let [[heroMode setHeroMode] (react/useState true)
        [reduced-motion set-reduced-motion] (react/useState false)
    ...
    (react/useEffect
     (fn []
       (let [media (js/window.matchMedia "(prefers-reduced-motion: reduce)")]
         (set-reduced-motion (.-matches media))
         (fn [] nil)))
     #js [])
    ...
    #jsx [:div {:className "relative h-screen w-full overflow-hidden bg-bg"}
          ...
          [AnimatePresence
           (when heroMode
             #jsx [motion.div {:className "fixed inset-0 z-20 flex flex-col items-center justify-end pb-32 px-4 pointer-events-none"
                               :initial {:opacity 1}
                               :exit {:opacity 0 :transition {:duration (if reduced-motion 0 0.5)}}}
                   ...
                   [motion.div {:className "absolute bottom-12 animate-bounce text-white pointer-events-none"
                                :initial {:opacity 1}
                                :exit {:opacity 0}}
          ...
          [motion.div {:ref dashboardRef
                       ...
                       :transition (if reduced-motion
                                     {:duration 0}
                                     {:type "spring" :stiffness 30 :damping 15 :mass 1.2})}
