...
(defn BlogPostView [{:keys [slug on-back]}]
  (let [{:keys [blogs]} (blog-router/use-blog-router)
        selected-blog (-> blogs
                          (.find (fn [blog] (= (:slug blog) slug))))
        path (when selected-blog (:path selected-blog))
        {:keys [content loading error]} (org-parser/use-org-content path)]
    (cond
      (not selected-blog)
      #jsx [:div {:className "h-full flex flex-col p-8 md:p-12 overflow-y-auto"}
            [:button {:className "mb-6 text-accent-blue hover:underline self-start"
                      :onClick on-back}
             "← Back to blog"]
            [:div {:className "text-red-500 text-center py-8"} (str "Blog post not found: " slug)]]
      ...
      #jsx [:div {:className "h-full flex flex-col p-8 md:p-12 overflow-y-auto"}
            ...
            [:div {:className "max-w-3xl w-full mx-auto prose prose-lg prose-slate max-w-none prose-headings:text-text-primary prose-p:text-text-secondary prose-strong:text-text-primary prose-a:text-accent-blue prose-a:no-underline hover:prose-a:underline prose-ul:text-text-secondary prose-li:text-text-secondary"
                   :dangerouslySetInnerHTML {:__html (or content "")}}]])))

(defn BlogListView [{:keys [on-select]}]
  (let [{:keys [blogs]} (blog-router/use-blog-router)]
    #jsx [:div {:className "h-full flex flex-col p-8 md:p-12 overflow-y-auto"}
                  ...
                  (-> blogs
                      (.map (fn [blog]
                              #jsx [BlogCard {:key (:slug blog)
                                              :title (:title blog)
                                              :slug (:slug blog)
                                              :on-click #(on-select (:slug blog))}])))])]))

(defn icon-expanded []
  (let [[current-slug set-current-slug] (react/useState nil)]
    (if current-slug
      #jsx [BlogPostView {:slug current-slug
                          :on-back #(set-current-slug nil)}]
      #jsx [BlogListView {:on-select set-current-slug}])))
