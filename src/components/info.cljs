...
(defn info-expanded []
  #jsx [:div {:className "p-8 md:p-12 flex flex-col items-center text-center"}
        [:div {:className "relative mb-8"}
         ...
         [:img {:src memoji :alt "Desmond Wang profile photo" :className "relative w-48 h-48 md:w-64 md:h-64 object-cover drop-shadow-2xl"}]]
        
        [:h1 {:className "text-4xl md:text-6xl font-bold text-text-primary mb-4 tracking-tight"} "Desmond Wang"]
        [:p {:className "text-lg text-text-secondary mb-2"} "Pengcheng Wang"]
...
(defn info [{:keys [layoutId className]}]
  #jsx [motion.div {:layoutId layoutId
        ...
        [:div {:className "relative shrink-0"}
         ...
         [:img {:src memoji :alt "Desmond Wang profile photo" :className "relative w-40 h-40 md:w-48 md:h-48 object-cover drop-shadow-xl"}]]
